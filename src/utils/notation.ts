...
import type { Notation, SquareInfo } from '~/utils/ts-helpers'
...
export function mapIndicesToNotation(row: number, column: number): Notation {
    ...
    const intermediateRow = squares[row]
    invariant(intermediateRow, 'Row does not exist.')
    const square = intermediateRow[column]
    invariant(square, 'Column does not exist.')
    return square
}
